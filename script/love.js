...
function init() {
  ...
  var koef = 1; // теперь canvas всегда полный размер
  ...
  for (var i = 0; i < heartPointsCount; i++) {
    ...
    e[i] = {
      ...
      vy: 0,
      R: 2,
      speed: mobile ? 0.8 + rand() * 0.3 : rand() + 5,
      q: ~~(rand() * heartPointsCount),
      D: 2 * (i % 2) - 1,
      force: mobile ? 0.96 + rand() * 0.03 : 0.2 * rand() + 0.7,
      f: "rgb(255, 33, 33)",
  ...
  var config = {
    traceK: mobile ? 0.25 : 0.4,
    timeDelta: mobile ? 0.2 : 0.6,
  };
  ...
  function loop() {
    ...
    time += (Math.sin(time) < 0 ? 6 : n > 0.8 ? 0.2 : 1) * config.timeDelta;
